...
import { formatCurrency, formatTime, formatDate, getOrderStatusInfo } from '@/lib/utils'
import { cn } from '@/lib/utils'
import toast from 'react-hot-toast'
import { CheckCircle, XCircle, ChefHat, Bell, Package, Search } from 'lucide-react'
...
const NEXT_ACTION: Record<string, { label: string; icon: typeof ChefHat; color: string }> = {
  ...
  ready:     { label: 'Mark Completed',  icon: Package,  color: 'bg-zinc-800 text-gray-400 border-zinc-700'   },
}
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-sm text-gray-400 space-y-0.5">
                      ...
                      {order.slot_date && order.slot_time && (
                        <p className="text-yellow-500 font-medium">
                          Pickup: {formatDate(order.slot_date)} at {formatTime(order.slot_time)}
                        </p>
                      )}
                      {order.order_type && (
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-yellow-500 font-bold text-xl">{formatCurrency(order.total_amount)}</p>
                ...
                <div className="bg-black/30 rounded-xl p-4 mb-4">
                  <p className="text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Order Items</p>
                  ...
                  {order.special_instructions && (
                    <div className="mt-3 pt-3 border-t border-zinc-800">
                      <p className="text-gray-400 text-xs font-medium mb-1">Special Instructions:</p>
                      <p className="text-white text-sm">{order.special_instructions}</p>
                    </div>
                  )}
                </div>
                ...
                {order.rejection_reason && (
                  <div className="mt-3 p-3 bg-red-500/10 border border-red-500/30 rounded-xl">
                    ...
                    <p className="text-red-300 text-sm">{order.rejection_reason}</p>
                  </div>
                )}
              </div>
            )
