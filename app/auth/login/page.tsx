  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) {
      if (error.message.toLowerCase().includes('invalid login credentials') ||
        ...
        setAuthError('This email is registered but the password is incorrect. Please try again or reset your password.')
      } else if (error.message.toLowerCase().includes('email not confirmed')) {
        setAuthError('Your email is not verified yet. Please check your inbox for a confirmation link.')
...
{authError && (
  ...
  >
    <span style={{ color: '#D94B4B', fontWeight: 600 }}>⚠ Login failed — </span>
    {authError}
  </div>
)}
              {/* Email */}
            ...
            >
              <div className="flex-1 h-px" style={{ backgroundColor: 'rgba(232,225,207,0.08)' }} />
              <span
